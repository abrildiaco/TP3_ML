...
class MLP:
    def __init__(self, input_size, hidden_size, output_size, hidden_nodes, hidden_activation = "relu",
                 output_activation = "softmax", learning_rate = 0.01, seed = 42):
    ...
    def fit(self, X, y, X_val = None, y_val = None, epochs = 10, verbose = True):
        ...
        for epoch in range(epochs):
            y_proba, pre_activations, activations = self.foward_pass(X)

            train_loss = self.cross_entropy_loss(y, y_proba)
            gradients = self.backward_pass(X, y, pre_activations, activations)
            ...
            if X_val is not None and y_val is not None:
                ...
                val_loss = self.cross_entropy_loss(y_val, y_val_proba)
